...
export function createEmailHtml(data: EmailData): string {
  const { name, role, temporaryPassword, loginUrl, isNewUser, organizationCode } = data;
      ...
      <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>
              <a href="${organizationCode ? `${loginUrl}?org=${organizationCode}` : loginUrl}" style="background-color: #4CAF50; color: white; padding: 10px 15px; text-decoration: none; border-radius: 5px; display: inline-block;">
                Set Up Your Password
              </a>
            </p>
            <p>For security reasons, we recommend changing this temporary password immediately after logging in.</p>
            <p>If the button above doesn't work, copy and paste this URL into your browser:</p>
            <p style="word-break: break-all; font-size: 12px;">${organizationCode ? `${loginUrl}?org=${organizationCode}` : loginUrl}</p>
