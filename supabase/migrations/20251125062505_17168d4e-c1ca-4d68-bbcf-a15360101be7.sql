-- Function to clean up practice leader assignments when properties are unassigned from managers
...
DROP TRIGGER IF EXISTS trigger_cleanup_practice_leader_on_unassignment ON public.profiles;
CREATE TRIGGER trigger_cleanup_practice_leader_on_unassignment
  AFTER UPDATE OF assigned_properties ON public.profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.cleanup_practice_leader_on_property_unassignment();