-- Fix public maintenance request function to handle attachments
...
BEGIN
  ...
  INSERT INTO public.maintenance_requests (
    ...
    property_name,  -- Set site to property name
    'pending',
    p_submitted_by,
    ...
    p_attachments,  -- Store the attachments
    now(),
    now()
  ...
  INSERT INTO public.activity_logs (
    ...
    jsonb_build_object(
      ...
      'attachments_count', CASE WHEN p_attachments IS NULL THEN 0 ELSE jsonb_array_length(p_attachments) END
    ),
    now()
  );
  ...
  RAISE LOG 'Public maintenance request submitted: ID=%, Property=%, Contact=%, Attachments=%', 
    request_id, p_property_id, p_contact_email, 
    CASE WHEN p_attachments IS NULL THEN 0 ELSE jsonb_array_length(p_attachments) END;
