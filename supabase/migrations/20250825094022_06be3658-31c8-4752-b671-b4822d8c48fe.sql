-- Fix the email notification function to use proper service role key authentication
...
BEGIN
  -- Get the service role key from pg_settings or use a fallback approach
  -- Since we can't directly access Supabase secrets from DB functions,
  -- we'll use the supabase service role key from the vault if available
  BEGIN
    SELECT current_setting('app.supabase_service_role_key', false) INTO service_role_key;
  EXCEPTION
    WHEN OTHERS THEN
      -- Log the issue and return early if no service key is available
      RAISE NOTICE 'Service role key not configured for email notifications';
      RETURN;
  END;
...
DO $$
BEGIN
  -- Set the service role key setting for the session
  -- In production, this should be set at the database level by an admin
  PERFORM set_config('app.supabase_service_role_key', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx0amxzd3pyZGd0b2RkeXFteWRvIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0NDU0MDk5MiwiZXhwIjoyMDYwMTE2OTkyfQ.BLQ5g9hPWdNcWfSBALa5wE2_5qVdYJV7sQjfmUgV0t4', false);
END $$;