-- Remove the webhook function if it exists and create a proper database trigger instead
-- This ensures email confirmations are sent when users sign up
...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NEW.email_confirmed_at IS NULL THEN
    -- Create the confirmation URL using the confirmation token
    confirmation_url := 'https://ltjlswzrdgtoddyqmydo.supabase.co/auth/v1/verify?token=' || 
                       NEW.confirmation_token || 
                       '&type=signup&redirect_to=' || 
                       encode('https://ltjlswzrdgtoddyqmydo.supabase.co/email-confirm', 'escape');
    
    RAISE NOTICE 'Sending confirmation email to: % with URL: %', NEW.email, confirmation_url;
    
    -- Call the send-auth-email edge function with proper webhook format
