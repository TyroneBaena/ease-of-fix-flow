-- Add managers to comment notification recipients

-- Update the create_comment_notifications function to include managers assigned to the property
...
BEGIN
  ...
  -- Notify managers assigned to the property (if property_id exists)
  IF request_record.property_id IS NOT NULL THEN
    FOR manager_user IN 
      SELECT id FROM public.profiles 
      WHERE role = 'manager' 
      AND organization_id = request_org_id
      AND request_record.property_id = ANY(assigned_properties)
    LOOP
      INSERT INTO public.notifications (user_id, title, message, type, link, organization_id)
      VALUES (manager_user.id, notification_title, notification_message, 'info', notification_link, request_org_id)
      ON CONFLICT DO NOTHING;
    END LOOP;
  END IF;
