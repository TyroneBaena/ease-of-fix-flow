-- Fix the HTTP response field access and confirmation token handling
...
BEGIN
  ...
  IF TG_OP = 'INSERT' AND NEW.email_confirmed_at IS NULL THEN
    ...
    RAISE LOG 'Available confirmation token: %', NEW.confirmation_token;
    
    -- Build confirmation URL with proper token handling
    confirmation_url := 'https://ltjlswzrdgtoddyqmydo.supabase.co/auth/v1/verify?token=' || 
                       COALESCE(NEW.confirmation_token, '') || 
    ...
    BEGIN
      SELECT * INTO http_response FROM net.http_post(
        ...
        body := jsonb_build_object(
          ...
          'record', jsonb_build_object(
            'id', NEW.id,
            'email', NEW.email,
            'email_confirm_token', COALESCE(NEW.confirmation_token, ''),
      ...
      -- Fix: Use correct field name status_code instead of status
