...
export function useComments(requestId: string) {
  ...
  const fetchComments = useCallback(async () => {
    ...
    console.log(`⏳ v97.1 - useComments: Waiting for session version ${sessionVersion} to be ready...`);
    const isReady = await waitForSessionReady(sessionVersion, 10000);

    if (!isReady) {
      console.warn("⚠️ v97.1 - useComments: Session not ready after timeout");
      setIsLoading(false);
      return;
    }
    ...
    setIsLoading(true);
    try {
      console.log(`✅ v97.1 - useComments: Session version ${sessionVersion} ready, fetching comments`);
      const { data, error } = await supabase
        .from("comments")
        .select("*")
        .eq("request_id", requestId)
