import React, { useMemo } from 'react';
import { useNavigate } from 'react-router-dom';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { AlertTriangle, ChevronRight } from 'lucide-react';
import { MaintenanceRequest } from '@/types/maintenance';
import { differenceInDays } from 'date-fns';

interface PendingActionsWidgetProps {
  requests: MaintenanceRequest[];
}

interface AgingRequest {
  request: MaintenanceRequest;
  daysOld: number;
}

export const PendingActionsWidget: React.FC<PendingActionsWidgetProps> = ({ requests }) => {
  const navigate = useNavigate();

  const { agingRequests, highPriorityCount, breakdown } = useMemo(() => {
    const now = new Date();
    const pendingOrOpen = requests.filter(
      r => r.status === 'pending' || r.status === 'open'
    );

    // Calculate aging for each request
    const aging: AgingRequest[] = pendingOrOpen
      .map(request => ({
        request,
        daysOld: differenceInDays(now, new Date(request.createdAt)),
      }))
      .filter(item => item.daysOld >= 3)
      .sort((a, b) => b.daysOld - a.daysOld);

    // High priority unassigned requests count
    const highPriority = pendingOrOpen.filter(
      r => (r.priority === 'high' || r.priority === 'critical') && 
           !r.contractorId && 
           !r.assigned_to_landlord
    ).length;

    // Age breakdown
    const breakdown = {
      days3to7: aging.filter(a => a.daysOld >= 3 && a.daysOld < 7).length,
      days7to14: aging.filter(a => a.daysOld >= 7 && a.daysOld < 14).length,
      days14plus: aging.filter(a => a.daysOld >= 14).length
    };

    return {
      agingRequests: aging,
      highPriorityCount: highPriority,
      breakdown
    };
  }, [requests]);

  // Don't render if there are no pending actions
  if (agingRequests.length === 0 && highPriorityCount === 0) {
    return null;
  }

  const handleViewAll = () => {
    navigate('/requests?status=open');
  };

  const handleRequestClick = (requestId: string) => {
    navigate(`/requests/${requestId}`);
  };

  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case 'critical': return 'bg-red-100 text-red-800';
      case 'high': return 'bg-orange-100 text-orange-800';
      case 'medium': return 'bg-yellow-100 text-yellow-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };

  const getAgeBadgeColor = (days: number) => {
    if (days >= 14) return 'bg-red-100 text-red-700';
    if (days >= 7) return 'bg-orange-100 text-orange-700';
    return 'bg-amber-100 text-amber-700';
  };

  return (
    <Card className="border-amber-200 bg-amber-50/50">
      <CardHeader className="pb-2 pt-3 px-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <AlertTriangle className="h-4 w-4 text-amber-600" />
            <CardTitle className="text-sm font-semibold text-amber-900">
              Pending Actions
            </CardTitle>
          </div>
          <Button 
            variant="ghost" 
            size="sm" 
            onClick={handleViewAll}
            className="h-7 px-2 text-amber-700 hover:bg-amber-100"
          >
            <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
      </CardHeader>
      <CardContent className="pt-0 px-4 pb-3">
        {/* Compact Summary */}
        <div className="text-xs text-amber-800 mb-2">
          <span className="font-medium">{agingRequests.length}</span> aging
          {highPriorityCount > 0 && (
            <> • <span className="font-medium text-orange-700">{highPriorityCount}</span> high priority</>
          )}
        </div>
        
        {/* Age Breakdown - single line */}
        <div className="flex gap-2 text-xs text-muted-foreground mb-3">
          <span><span className="font-medium text-amber-600">{breakdown.days3to7}</span> 3-7d</span>
          <span><span className="font-medium text-orange-600">{breakdown.days7to14}</span> 7-14d</span>
          <span><span className="font-medium text-red-600">{breakdown.days14plus}</span> 14d+</span>
        </div>

        {/* Compact Request List - max 3 items */}
        {agingRequests.length > 0 && (
          <div className="space-y-1.5">
            {agingRequests.slice(0, 3).map(({ request, daysOld }) => (
              <div 
                key={request.id}
                onClick={() => handleRequestClick(request.id)}
                className="flex items-center justify-between gap-2 bg-white rounded p-2 border border-amber-200 hover:border-amber-400 cursor-pointer transition-colors"
              >
                <p className="text-xs font-medium text-foreground truncate flex-1">
                  {request.title}
                </p>
                <div className="flex items-center gap-1 flex-shrink-0">
                  <Badge className={`${getAgeBadgeColor(daysOld)} text-[10px] px-1.5 py-0`} variant="outline">
                    {daysOld}d
                  </Badge>
                  <Badge className={`${getPriorityColor(request.priority)} text-[10px] px-1.5 py-0`} variant="outline">
                    {request.priority.slice(0, 3)}
                  </Badge>
                </div>
              </div>
            ))}
            {agingRequests.length > 3 && (
              <p className="text-[10px] text-amber-600 text-center">
                +{agingRequests.length - 3} more
              </p>
            )}
          </div>
        )}
      </CardContent>
    </Card>
  );
};
