...
// src/integrations/supabase/client.ts
// v61.0 - Corrected cookie-based session logic (NO autoRefreshToken)
...
export function getSupabaseClient(): SupabaseClient<Database> {
  if (!_supabase) {
    console.log("🔧 v61.0 - Creating SINGLE Supabase client ( cookies only )");

    _supabase = createClient<Database>(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: {
        // ❗ Cookies only. Do NOT persist anything on the client.
        persistSession: false,

        // ❗ VERY IMPORTANT:
        // For cookie-based auth, Supabase MUST NOT auto-refresh tokens
