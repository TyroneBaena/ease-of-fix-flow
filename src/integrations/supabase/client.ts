...
function restoreSessionFromBackup(): Session | null {
  const local = localStorage.getItem(BACKUP_KEY);
  const cookie = getCookie(COOKIE_NAME);
  const source = local || cookie;
...
// ------------------------
// Force reconnect Realtime safely
// ------------------------
export async function reconnectRealtime() {
  try {
...
async function ensureSessionAlive() {
  try {
    ...
    if (error || !data.session) throw new Error("Missing session");
    ...
    if (expiresAt && expiresAt < now + 60) {
      ...
      if (refreshed?.session) backupSession(refreshed.session);
    }
  } catch {
    console.log("⚠️ Session invalid, attempting restore via refresh_token...");
    const backup = restoreSessionFromBackup();
    if (backup?.refresh_token) {
      ...
      else if (data?.session) backupSession(data.session);
    }
  }
}

// ------------------------
// Keepalive & visibility listeners
// ------------------------
function setupKeepAlive() {
  // Refresh every 5 minutes
  setInterval(ensureSessionAlive, 5 * 60_000);

  // Also on tab revisit or focus
  const onVisible = async () => {
    if (document.visibilityState === "visible") {
      console.log("👁️ Tab became visible, verifying session...");
  ...
  window.addEventListener("online", ensureSessionAlive);
}

// Initialize once
(async () => {
  console.log("🚀 Initial Supabase session init");
  await ensureSessionAlive();
  setupKeepAlive();
})();
