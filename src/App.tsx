...
// Import the coordinator for optional direct debugging
import { visibilityCoordinator } from '@/utils/visibilityCoordinator';

// Configure QueryClient (disable auto refetch on tab focus)
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      refetchOnWindowFocus: false,
...
function AppRoutes() {
  ...
  React.useEffect(() => {
    const failsafe = setTimeout(() => {
      if (loading && !forceReady) {
        console.error('⚠️ AppRoutes - Loading timeout exceeded! Forcing app render.');
        setForceReady(true);
      }
    }, 8000);
...
function App() {
  // Optionally, monitor coordinator globally (for debugging)
  React.useEffect(() => {
    console.log('🧠 VisibilityCoordinator active:', visibilityCoordinator.getState());
  }, []);
